...
diag_sudoku = 1 # Set this flag = 0 for non-diagonal sudoku
if diag_sudoku == 1:
...
def naked_twins(values):
    ...
    # First select boxes with twin values
    twin_vals = [box for box in values.keys() if len(values[box]) == 2]
    print('\n all boxes with twin vals here"')
    print(twin_vals)# Naked Twins - boxes with same elements

    # Naked Twins - boxes with same elements
    naked_twins = []
    for box1 in twin_vals:
        for box2 in peers[box1]:
            if set(values[box1])==set(values[box2]):
                naked_twins.append([box1,box2])
    print('\n naked twins  - alt here')
    print(naked_twins)
...
def reduce_puzzle(values):
    ...
    while not stalled:
        ...
        print('\n Suduko board BEFORE naked twins optimization:')
        display(values)
        values = naked_twins(values)
        print('\n Suduko board AFTER naked twins optimization:')
        display(values)
